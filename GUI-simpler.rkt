...
; procedure initialize. After their values are assigned variables viewport and escape never are
; mutated again. The viewport can not yet be assigned because this needs graphics to be open.
; Graphics is opened by procedure initialize which also will open and assign the viewport.
...
(define escape       'delayed) ; Assigned one time only.
(define viewport     'delayed) ; Assigned one time only.
...
(define (initialize ec)
  ...
  ; Reinitialize buttons with content.
...
; For the procedure property of buttons without content.
...
; For the procedure property of buttons with content.
