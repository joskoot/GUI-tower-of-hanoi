...
(define (tower-of-hanoi)
  ...
  (let/ec escape
    ...
    (define (initialize)
      ...
      ; Also initializes variable disk-distr.
      (action-reset))
    ...
    (define (proc-button2 button action (arg #f))
      (case action ; For the procedure property of buttons with content.
        ((get-content)
    ...
    ;=================================================================================================
    ; Computation of the dimensions of buttons. These depend on the sizes of the texts the buttons
    ; must hold. The sizes are given by procedure get-string-size. A temporary pixmap is used. This
    ; requires graphics, which is temporarily opened too. Both will be closed after finishing the
    ; computation.
    ...
    ;===============================================================================?=================
    ; When validating a modal dialog that returns a string, data must be read from the string,
    ; possibly followed by some computation. We don't want tje GUI to crash when the user provides
    ; wrong data. The validator just must reject the answer given by the user. 
